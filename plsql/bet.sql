...
procedure sp_Page(  pageSize       int, 
  ...
  --执行自动跟单
  procedure  autoGendan(sponsorBetOrderId in number,sponsorUserId in number,pSingleMoney in number,pType  in number)
  is
  ...
  begin
    for v in (select  my.USERID,my.TX_MONEY  from t_My_Auto_Order my  order by my.order_time)  loop
       txMoney:=v.tx_money;
       select va.all_money,va.frozen_money,va.user_name into  allMoney,frozenMoney,betUsername  from t_virtual_account  va  
       where va.tx_user_id=v.userid  and va.status='1';
       ...
       if txMoney>=pSingleMoney then--只有跟单金额大于一份的金额时才能跟单
        ...
        if availiableMoney>=txMoney then
          ...
          if copy+alreadyBuyCopys<=allCopys then
              insert into  T_BET_ORDER(BET_ID,BET_USERID,SUBSCRIBE_MONEY,BET_TIME,SPONSOR_TYPE,SPONSOR_BET_ID,TYPE,BET_USERNAME)
              values(SEQ_TOTAL_GOAL_ID.NEXTVAL,v.userid,txMoney,sysdate,'2',sponsorBetOrderId,pType,betUsername);
