...
public class DuplexShoppingHalfComplete6Action extends BaseAction {
	...
	public String display() {
		...
		try {
			...
			parameter1.put("phaseCategory",
					Constants.ORDER_CATEGORY_SIXHALFCOMPLETE);
			...
			if (user != null && !"".equals(user)) {
				...
				if (!CommUtil.matcher(CommUtil.betMultiRegEx, betMulti.trim())) {
					...
					if ("primary".equals(actionType)) {// 用户手选式投注处理。
						...
						/* 验证用户传来的投注方案 */
						if (!CommUtil.matcher(CommUtil.halfComplete6RegEx, plan
								.trim())) {
							...
							for (int i = 0; i < palnArray.length; i++) {
								String[] innerArray = palnArray[i].split("-");
								for (int j = 0; j < innerArray.length; j++) {
									count *= innerArray[j].length();
								}
							}
							// 获得用户所投注的倍数
						...
						if ("".equals(message.toString())) {
							...
							// 保存文件的路径
							setDir(targetDirectory + "\\" + 5 + "\\" + phaseId + "\\" + targetFileName);
							// 建立一个目标文件
							File target = new File(targetDirectory + "\\" + 5 + "\\" + phaseId, targetFileName);
	...
	private void doValidator(StringBuilder message, JSONObject jsonResult) {
		// 设定上传文件最大值
		int uploadFileMaxSize = (12 * 2 - 1) * 531441;
		// 用户所投注数
		String betNum = "0";
		int caculatBetNum = 0;
		try {
				...
				if (doc.length() < uploadFileMaxSize) {
					...
					if (!".txt".equals(extension)) {
						...
						while (di.hasNext()) {
							...
							if (!CommUtil.matcher(
									CommUtil.halfComplete6UpLoadRegEx, line)) {
								message
