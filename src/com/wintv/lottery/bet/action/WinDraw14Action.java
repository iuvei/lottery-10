...
public class WinDraw14Action extends BaseAction {
			...
			if (user != null && !"".equals(user)) {
				...
				if (!CommUtil.matcher(CommUtil.betMultiRegEx, betMulti.trim())) {
					...
					if ("primary".equals(actionType)) {// 用户手选式投注处理。
						...
						/* 验证用户传来的投注方案 */
						if (!CommUtil.matcher(CommUtil.winDraw14RegEx, plan
								.trim())) {
						...
						if ("".equals(message.toString())) {
							...
							// 保存文件的路径
							setDir(targetDirectory + "\\" + 1 + "\\" + phaseId + "\\" + targetFileName);
							// 建立一个目标文件
							File target = new File(targetDirectory + "\\" + 1 + "\\" + phaseId, targetFileName);
	...
	private void doValidator(StringBuilder message, JSONObject jsonResult) {
		// 设定上传文件最大值
		int uploadFileMaxSize = (14 * 2 - 1) * 4782969;
		// 用户所投注数
		String betNum = "0";
		int caculatBetNum = 0;
		try {
				...
				if (doc.length() < uploadFileMaxSize) {
					...
					if (!".txt".equals(extension)) {
						...
						while (di.hasNext()) {
							...
							if (!CommUtil
									.matcher(CommUtil.winDraw14RegEx, line)) {
								message
