...
public class DuplexShoppingGoal4Action extends BaseAction {
	...
	public String display() {
		...
		try {
			...
			parameter1.put("phaseCategory",
					Constants.ORDER_CATEGORY_FOURSCENEGOAL);
			...
			if (user != null && !"".equals(user)) {
				...
				if (!CommUtil.matcher(CommUtil.betMultiRegEx, betMulti.trim())) {
					...
					if ("primary".equals(actionType)) {// 用户手选式投注处理。
						...
						// 去除“+”
						plan = plan.trim().replace("+", "");
						if (!CommUtil.matcher(CommUtil.goal4RegEx, plan)) {
						...
						if ("".equals(message.toString())) {
							...
							// 保存文件的路径
							setDir(targetDirectory + "\\" + 3 + "\\" + phaseId + "\\" + targetFileName);
							// 建立一个目标文件
							File target = new File(targetDirectory + "\\" + 3 + "\\" + phaseId, targetFileName);
				...
				if (doc.length() < uploadFileMaxSize) {
					...
					if (!".txt".equals(extension)) {
						...
						while (di.hasNext()) {
							...
							if (!CommUtil.matcher(CommUtil.goal4UpLoadRegEx,
									line)) {
								message
