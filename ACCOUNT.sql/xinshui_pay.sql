...
--返回 1表示结算成功
procedure  executeXinshuiResult(resultValue OUT  NUMBER)--2010 03-01 09:17 心水赛果出来后  进行结算
is 
select_type CHAR(1);--民间高手发布的心水结果
xinshui_result CHAR(2);--心水赛果
...
begin
   for v in(select b.c2c_id,a.xinshui_result,b.select_type,b.Ensure_Money,b.price,b.tx_username from T_XINSHUI_AGAINST a,T_C2C_PRODUCT b  where a.AGAINST_ID=b.AGAINST_ID  
   and a.XINSHUI_RESULT is not null)  loop
        ...
        if buyUserCnt=0 then--如果没有人购买则返回-1
          --解冻民间高手的保证金
          ...
          --插入解冻日志
          insert into t_Va_Frozen_Log(FROZEN_ID,Tx_User_Id,Frozen_Money,Frozen_Date,Memo,Order_Id,Category_Type,Order_No,Flg)
          values(SEQ_VA_FROZEN_LOG_ID.Nextval,minjianUserId,ensureMoney,sysdate,'心水退款',c2cId,'15',orderNO,'2');
          --对于
        else
